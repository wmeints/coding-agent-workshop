...
In this lab, you will extend your coding agent by adding a shell plugin that
...
You'll learn how to create platform-specific plugins that adapt to different
...
## Creating the Unix shell plugin

The first plugin you'll build executes commands using Bash on Unix-based systems
(macOS and Linux). This plugin will handle process execution, environment
variable configuration, and output collection.

<Steps>

1. Create a new directory `Plugins/Shell` in your CodingAgent project.
...
4. In the function description, include the instructions shown below in the `Description` attribute.
...
## Creating the Windows shell plugin

Next, you'll create a similar plugin for Windows systems that uses CMD instead of Bash.
...
4. Use the same approach for environment variables, but adapt the syntax for Windows batch commands.
...
## Creating the shell plugin factory

To ensure the correct plugin is used based on the operating system, you'll create a factory class that
...
## Registering the shell plugin with the agent

Finally, you'll integrate the shell plugin into your agent so it can use the shell functionality.

<Steps>

1. Open the `Agent.cs` file in your CodingAgent project.

2. In the constructor of the `Agent` class, after initializing the kernel and other dependencies,
   register the shell plugin by calling `_kernel.Plugins.Add(ShellPluginFactory.Create())`.

3. Add the necessary using statement for the `CodingAgent.Plugins.Shell` namespace.

</Steps>

## Testing the shell plugin

Now that you've implemented the shell plugin, test it to ensure the agent can execute commands.
...
In this lab, you built a cross-platform shell plugin for your coding agent. You created platform-specific
implementations for Unix and Windows systems, used a factory pattern to select the correct implementation,
and registered the plugin with Semantic Kernel. This enables your agent to execute commands, interact with
