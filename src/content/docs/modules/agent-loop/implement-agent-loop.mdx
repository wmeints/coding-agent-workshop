...
2. In the main loop, invoke the chat completion service to get a response to
   ...
   ```csharp title="Agent.cs" ins={18-26}
   public async Task InvokeAsync(string prompt, IAgentCallbacks callbacks)
   {
      var iterations = 0;
      var chatCompletionService = _kernel.GetRequiredService<IChatCompletionService>();
...
4. The `HandleFunctionCalls` method is responsible for calling functions and reporting
   ...
   <Aside type="tip" title="Why are we not using the built-in tool invocation?">
...
1. Create a new class `Plugins/SharedToolsPlugin.cs` in the `CodingAgent` project.
2. Add the following using statements to the top of the file:

   ```csharp title="Plugins/SharedToolsPlugin.cs"
   using System.ComponentModel;
   using Microsoft.SemanticKernel;
   ```

3. Next, add two properties to the class to store the final output and a flag
   indicating whether the final output tool was called and the final output to
...
4. Finally, add the `FinalOutputAsync` method to the class. This method will
   be the tool that the agent can call to signal that it is done.

   ```csharp title="Plugins/SharedToolsPlugin.cs" ins={12-23}
   public class SharedToolsPlugin
   {
      public bool TaskCompleted { get; private set; }
      public string FinalOutput { get; private set; } = "";
...
When you test the agent in the current state, it will work, but you can't see
anything. Also, it may be very slow to get back to the prompt because the agent
doesn't always use the final output tool. We need to provide it with proper
instructions to make sure it uses the tool correctly.

## Add agent instructions

Let's add logic to instruct the agent to use the final output tool. The logic we
write here will also be important if you want to use custom instructions later.
...
1. Download [SystemInstructions.txt]() and place it in the `Resources` folder
   in the `CodingAgent` project. Make sure to set the file's build action to
...
1. Create the method `ReadSystemInstructionsAsync` in the `AgentInstructions`
   ...
   {
      var resourceName = "CodingAgent.Resources.SystemInstructions.txt";
...
1. Modify the `InvokeAsync` method in the `Agent` class to inject and
   ...
   ```csharp title="Agent.cs" ins={6,42}
   public async Task InvokeAsync(string prompt, IAgentCallbacks callbacks)
   {
      var iterations = 0;
      var chatCompletionService = _kernel.GetRequiredService<IChatCompletionService>();
