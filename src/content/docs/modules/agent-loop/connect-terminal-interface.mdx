...
- **Starter:** [labs/agent-loop/002-main-agent-loop][LAB_START_SOURCE]
- **Solution:** [labs/agent-loop/003-agent-interface][LAB_SOLUTION_SOURCE]
...
3. Implement the `ReportFunctionCallAsync` method:
   ...
   We'll report the function arguments as well, so that we can see what input
   the tool receives from the agent. This is useful when you're running shell
...
2. **Intermediate response reporting** - Shows agent responses that don't
   involve tool calls
...
Please find the final solution for the lab here:
[labs/agent-loop/003-agent-interface][LAB_SOLUTION_SOURCE].

In the next lab, we'll work on proper unit-tests for the agent so you don't need
to manually verify its behavior each time you make a change.

[LAB_START_SOURCE]: https://github.com/wmeints/coding-agent-workshop/tree/main/labs/agent-loop/002-main-agent-loop
[LAB_SOLUTION_SOURCE]: https://github.com/wmeints/coding-agent-workshop/tree/main/labs/agent-loop/003-agent-interface