...
- **Starter code**: `labs/context-engineering/001-custom-instructions/`
- **Solution code**: `labs/context-engineering/002-task-memory/`
...
It's okay if your subscription is shorter than the one shown above. The key is to
...
2. In the `OnFunctionInvokedAsync` method, add a new case to handle the
   `write_todos` function:
   ```csharp
   if(functionName == "write_todos")
   {
      ReportWriteTodosInvocation(JsonSerializer.Deserialize<List<TodoItem>>(arguments["todos"]));
      return;
   }
   ```

3. Implement the `ReportWriteTodosInvocation` method to format and display the
...
foundation for building capable coding agents. In the next module, you'll learn
how to add retrieval-augmented generation to give your agent access to external
knowledge sources.

**Solution**: `labs/context-engineering/002-task-memory/`
